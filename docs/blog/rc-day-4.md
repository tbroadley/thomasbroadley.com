...
Finally, I paired again on [`cssc`](https://github.com/stephen/cssc). We added support for reducing one species of multi-term `calc` expression, ones like `calc(100% - 10px - 10px)` that have multiple terms with the same units next to each other. We might pair again tomorrow to get `cssc` to handle expressions like `calc(100px - 10% - 10px)` where the terms with the same units are separated.
